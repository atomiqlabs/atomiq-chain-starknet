    ...
    static serializeTx(tx) {
        ...
        if (isStarknetTxInvoke(tx)) {
            const calls = tx.tx.map(call => ({
                ...call,
                calldata: starknet_1.CallData.compile(call.calldata),
            }));
            const signed = tx.signed == null ? undefined : {
                ...tx.signed,
                calldata: starknet_1.CallData.compile(tx.signed.calldata),
            ...
            const deployPaylod = {
                ...tx.tx,
                constructorCalldata: starknet_1.CallData.compile(tx.tx.constructorCalldata),
                addressSalt: (0, Utils_1.toHex)(tx.tx.addressSalt)
            };
            const signed = tx.signed == null ? undefined : {
                ...tx.signed,
                constructorCalldata: starknet_1.CallData.compile(tx.tx.constructorCalldata),
