...
class StarknetChainEventsBrowser {
    ...
    async checkEventsEcrowManager(lastTxHash, lastBlockNumber, currentBlock) {
        let events = await this.starknetSwapContract.Events.getContractBlockEvents(["escrow_manager::events::Initialize", "escrow_manager::events::Claim", "escrow_manager::events::Refund"], [], lastBlockNumber == null ? undefined : lastBlockNumber + 1, currentBlock?.block_number);
        if (lastTxHash != null) {
    ...
    async checkEventsSpvVaults(lastTxHash, lastBlockNumber, currentBlock) {
        let events = await this.starknetSpvVaultContract.Events.getContractBlockEvents(["spv_swap_vault::events::Opened", "spv_swap_vault::events::Deposited", "spv_swap_vault::events::Closed", "spv_swap_vault::events::Fronted", "spv_swap_vault::events::Claimed"], [], lastBlockNumber == null ? undefined : lastBlockNumber + 1, currentBlock?.block_number);
        if (lastTxHash != null) {
            const latestProcessedEventIndex = (0, Utils_1.findLastIndex)(events, val => val.txHash === lastTxHash);
            if (latestProcessedEventIndex !== -1)
                events.splice(0, latestProcessedEventIndex + 1);
    ...
    async checkEvents(lastBlockNumber, lastTxHashes) {
        ...
        //Get pending events
        lastTxHashes[0] = await this.checkEventsEcrowManager(lastTxHashes[0]);
        lastTxHashes[1] = await this.checkEventsSpvVaults(lastTxHashes[1]);
