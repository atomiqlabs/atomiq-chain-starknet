import {notNull, toBigInt, toBigIntSafe} from "../../../utils/Utils";
...
export class StarknetLpVault extends StarknetSwapModule {
    ...
    public async getIntermediaryReputation(address: string, token: string): Promise<IntermediaryReputationType> {
        ...
        const length = toBigIntSafe(rawReputation.shift());
        ...
        Object.keys(this.contract.claimHandlersByAddress).forEach((address) => {
            ...
            result[handler.getType()] = {
                successVolume: toBigInt({low: notNull(rawReputation.shift()), high: notNull(rawReputation.shift())}),
                successCount: toBigIntSafe(rawReputation.shift()),
                coopCloseVolume: toBigInt({low: notNull(rawReputation.shift()), high: notNull(rawReputation.shift())}),
                coopCloseCount: toBigIntSafe(rawReputation.shift()),
                failVolume: toBigInt({low: notNull(rawReputation.shift()), high: notNull(rawReputation.shift())}),
                failCount: toBigIntSafe(rawReputation.shift()),
            };
        });
